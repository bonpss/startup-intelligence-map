...
comment on column users.is_owner is 'True only for the single account whose signup email matched OWNER_EMAIL. Exempt from the MAX_USERS signup cap (storage.count_non_owner_users() excludes it) and from the /graph, /api/graph/* block enforced by graph_app.py''s auth-gating middleware.';
